...
router = APIRouter(tags= ["persona y movil"])
...
@router.get("/moviles", response_model=List[Movil])
def get_all_moviles():
    return moviles_list

@router.get("/moviles/{Id}", response_model=Movil)
...
def add_movil(movil_data: MovilInput):
    try:
        ...
        raise HTTPException(status_code=400, detail=f"IdPersona {movil_data.IdPersona} no existe")
...
@router.put("/moviles/{Id}", response_model=Movil)
